...
function dxCreateEdit(name,text,defaultText,x,y,w,h)
    ...
    if not fonts[fontsize] then 
        ...
        outputDebugString("Font "..fontsize.." létrehozva!");
    end
    edits[id] = {name,text,defaultText,x,y,w,h,fonts[fontsize],false,0,0,100,getTickCount()};
    outputDebugString("dxEditbox létrehozva. ID: "..id.." | Név: "..name,0,0,100,100);
    return id;
end
...
function dxDestroyEdit(id)
    if tonumber(id) then 
        if not edits[id] then error("Nem valódi editbox") return false end;
        table.remove(edits,id)
        outputDebugString("dxEditbox eltüntetve. ID: "..id,0,0,100,100);
        return true;
    else 
        local edit = dxGetEdit(id);
        if not edits[edit] then error("Nem valódi editbox") return false end;
...
--SCRIPT TEST--
local one = dxCreateEdit("teszt","","teszt",sx/2+150,sy/2,200,30,11,200);
local two = dxCreateEdit("teszt 2","","teszt 2",sx/2+150,sy/2+50,200,30,11,200);
---------------
